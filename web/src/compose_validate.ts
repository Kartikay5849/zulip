...
export function warn_if_mentioning_unsubscribed_group(
    ...
    const $banner_container = compose_banner.get_compose_banner_container($textarea);
    if (
        $banner_container.find(`.${CSS.escape(compose_banner.CLASSNAMES.user_not_subscribed)}`)
            .length > 0
    ) {
        return;
    }
    ...
    const context = {
        ...
        message: $t(
            {
                defaultMessage:
                    "None of the members of {group_name} are subscribed to this channel, so mentioning the group won't notify anyone.",
            },
            {group_name: mentioned_group.name},
        ),
    };
